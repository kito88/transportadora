...
from banco import conectar, criar_tabelas, adicionar_colunas_coletas
...
def consulta_cnpj():
    ...
    try:
        ...
        cidade = estabelecimento.get('municipio') or dados.get('cidade') or ''
        estado = estabelecimento.get('uf') or dados.get('estado') or ''
        telefone = estabelecimento.get('telefone1') or estabelecimento.get('telefone') or ''

        endereco_completo = f"{logradouro}, {numero}, {bairro}, {cidade} - {estado}".strip(', ')
...
def nova_coleta():
    ...
    if request.method == 'POST':
        # Limpar CNPJ do remetente
        remetente_cnpj = request.form.get('remetente_cnpj', '')
        remetente_cnpj = ''.join(filter(str.isdigit, remetente_cnpj))

        if not remetente_cnpj or len(remetente_cnpj) != 14:
            flash("CNPJ do remetente inválido.", "danger")
            return redirect(url_for('nova_coleta'))

        # Busca cliente pelo CNPJ
        cursor.execute(
            "SELECT id, nome, endereco, cidade, estado FROM clientes WHERE REPLACE(REPLACE(REPLACE(documento,'.',''),'/',''),'-','') = ?",
            (remetente_cnpj,)
        )
        cliente = cursor.fetchone()
        if not cliente:
            flash("Remetente não cadastrado. Cadastre antes de prosseguir.", "danger")
            conn.close()
            return redirect(url_for('novo_cliente'))

        cliente_id, remetente_nome, remetente_endereco, remetente_cidade, remetente_uf = cliente

        # Dados do destinatário
        destinatario_nome = request.form.get('destinatario_nome', '')
        destinatario_endereco = request.form.get('destinatario_endereco', '')
        destinatario_cidade = request.form.get('destinatario_cidade', '')
        destinatario_uf = request.form.get('destinatario_uf', '').upper()
        volumes = request.form.get('volumes', '')
        peso = request.form.get('peso', '')
        valor_mercadoria = request.form.get('valor_mercadoria', '')
        ...
        dimensoes = f"{largura}x{altura}x{comprimento}"
        volume_m3 = (largura/100) * (altura/100) * (comprimento/100)
        ...
        cursor.execute('''
            ...
            (cliente_id, destinatario_nome, destinatario_endereco, destinatario_cidade, destinatario_uf,
             volumes, peso, valor_mercadoria, largura, altura, comprimento, dimensoes, volume_m3, observacoes, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Pendente')
        ''', (cliente_id, destinatario_nome, destinatario_endereco, destinatario_cidade, destinatario_uf,
              volumes, peso, valor_mercadoria, largura, altura, comprimento, dimensoes, volume_m3, observacoes))
    ...
    # GET request: carrega clientes para dropdown
    cursor.execute("SELECT id, nome FROM clientes")
    clientes = cursor.fetchall()
    conn.close()
    return render_template('nova_coleta.html', clientes=clientes, cliente_id=None)
